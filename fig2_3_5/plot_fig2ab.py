...
for ik in range(ikmax):
    ...
    lidx = 0
    #errs[0,ik,0] = 2.0; errs[1,ik,0] = 2.0
    #for q in range(2):
    #    norms[q,ik,0] = dQR_init
...
plt.ylim(0.0, 0.6)
#plt.semilogy()
#plt.ylim(0.00000001, 100.0)#0.55)
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
plt.show()

svfg.savefig('fig_qbind_dat17g_readout2_la_N' + str(N) + '_Tp' + str(Tpmax) + '_ikm' + str(ikmax) + '.pdf')
...
svfg.savefig('fig_qbind_dat17g_readout2_lb_N' + str(N) + '_Tp' + str(Tpmax) + '_ikm' + str(ikmax) + '.pdf')
